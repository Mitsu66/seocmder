...
$strt = explode(" ",$str);
$return=array();
foreach($strt as $retour){
	$retour = trim($retour);
	$valid=true;
	if(strlen($retour)<3) { $valid=false; }
	foreach($words as $word)
	{
		if($word==$retour) { $valid=false;}
	}
	
	if($valid==true) { $return[] = $retour; }
}
return $return;
}
...
$str = "Découvrez plus de 150 astuces pour perdre du poids rapidement et naturellement sans utiliser des solutions medicamenteuses ! Il s agit d'astuces de grands-mère qui ont fait leurs preuves et que vous pouvez noter.
Découvrez dans le classement ci-dessous les meilleures astuces pour maigrir ! Il s agit d'une selection basée sur des criteres d'efficacite, ainsi que d'ingéniosite !";
